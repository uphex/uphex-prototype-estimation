...
describe UpHex::Prediction::TimeSpan do
  context "timespan initialization" do
    ...
    it 'creates a span from a hash (seconds)' do
      seconds = 30
      hash = {:seconds => 30}
      ts = described_class.from_hash(hash)
    ...
    it 'creates a span from two Time instances' do
      ...
      t0 = Time.now
      t1 = Time.now + diff
      ts = described_class.from_times(t0,t1)
      expect(ts.span).to eq(diff)
    end

    it "rounds fractional seconds down" do
      span = 30.131313
      ts = described_class.from_seconds(span)
  ...
  context "timespan initialize with odd values" do
    it "converts bad string values to numbers" do
      ts = described_class.from_seconds("bob")
      expect(ts.span).to eq 0.0
    end

    it "converts numeric strings to numbers" do
      ts = described_class.from_seconds("15")
    ...
    it "converts negative numbers to positive numbers" do
      ts = described_class.from_seconds(-30.0)
      expect(ts.span).to eq 30.0
    end

    it "handles bad types (array)" do
      expect{ described_class.from_seconds([])} .to raise_error(ArgumentError, /span type/)
    end

    it "handles bad types (hash)" do
      expect{ described_class.from_seconds([:test => "data"])} .to raise_error(ArgumentError, /span type/)
    end

    it "handles bad types (class)" do
      expect{ described_class.from_seconds(13.class)} .to raise_error(ArgumentError, /span type/)
    end

    it "handles bad types (span, string)" do
      expect {described_class.from_times(Time.now, "yesterday")}.to raise_error(ArgumentError, /time/)
    end

    it "handles bad types (span, array)" do
      expect {described_class.from_times(Time.now, [])}.to raise_error(ArgumentError, /time/)
    end

    it "handles bad types (array, hash)" do
      expect {described_class.from_times([], {})}.to raise_error(ArgumentError, /time/)
    end

    it "handles hases with bad keys" do
      expect {described_class.from_hash(:test => "data")}.to raise_error(NoMethodError)
    end

    it "handles hashes with bad values (non-numeric coercable)" do
      ts = described_class.from_hash(:days => "data")
      expect(ts.span).to eq 0.0
    end

    it "handles hashes with bad values (non-numeric not coercable)" do
      expect { described_class.from_hash(:days => [])}.to raise_error(ArgumentError, /span type/i)
    end
  end

  context "timespan advances along span" do
    it "advances a time by span and return a time" do
      seconds = 30
      t0 = Time.now
      ts = described_class.from_seconds(seconds)
      t1 = ts.advance(t0)
      expect(t1-t0).to eq seconds
    end

    it "fails when attempting to advance a non-time value (string)" do
      ts = described_class.from_seconds(10)
